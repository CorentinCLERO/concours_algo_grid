...
Action = Tuple[str, int, int, int, int, int]  # kind, x1, y1, x2, y2, color(-1 for joker)
Grid = List[List[int]]
...
def greedy_joker_prepass(
    ...
    w = len(target[0])
    used = 0
    all_jokers = iter_all_joker_rectangles(w, h, max_joker_size)
...
def apply_and_score(grid: Grid, target: Grid, action: Action) -> Tuple[Grid, int]:
    next_grid = clone_grid(grid)
    gain = compute_gain_for_action(next_grid, target, action)
    apply_action_in_place(next_grid, target, action)
    return next_grid, gain
...
def tiny_beam_construct(dataset: dict, seed: int) -> List[Action]:
    ...
    for _depth in range(1, max_actions):
        ...
        for actions, grid, jokers_used, correct in beam:
            ...
            # Echantillonne les rectangles les plus prometteurs via le gain,
            # puis complète avec quelques actions aléatoires pour diversifier.
            ...
            candidates: List[Action] = [action for _gain, action in scored_rects[:80]]
            ...
            for candidate in dedup.values():
                ...
                # Evite de diluer le beam avec des actions non informatives trop tôt.
...
def cli(default_dataset: int, description: str) -> None:
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--dataset", type=int, default=default_dataset, help=f"Dataset number (default: {default_dataset})")
