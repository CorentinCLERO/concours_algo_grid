...
def compress_with_rect_merge_reinsert(actions: List[Action], dataset: dict) -> List[Action]:
    max_jokers = dataset["maxJokers"]
    target = dataset["grid"]
    best = actions[:]

    def find_perfect_reordering_with_fixed_first(candidate: List[Action]) -> List[Action]:
        if not candidate:
            return candidate
        ...
        for perm in itertools.permutations(tail):
            ordered = [first, *perm]
            if is_perfect_against_target(ordered, target):
                return ordered
        return candidate
    ...
    while improved:
        ...
        for i in range(1, n):
            ...
            for j in range(i + 1, n):
                ...
                candidate = [act for idx, act in enumerate(best) if idx not in (i, j)] + [merged]
                if len(candidate) >= len(best):
                    continue
                if count_jokers(candidate) > max_jokers:
                    continue

                ordered = find_perfect_reordering_with_fixed_first(candidate)
                if is_perfect_against_target(ordered, target):
                    best = ordered
                    improved = True
                    break
...
def run_single_restart_dataset_1(dataset: dict, restart: int, iterations: int, seed: int) -> List[Action]:
    ...
    compressed = compress_with_rect_merge_reinsert(optimized, dataset)
    repaired = repair_with_single_swap(compressed, dataset)
    return compress_with_rect_merge_reinsert(repaired, dataset)


def find_best_solution_dataset_1(dataset: dict, iterations: int, restarts: int, seed: int, workers: int) -> List[Action]:
    best_actions: List[Action] = []
    best_score = -1

    if workers <= 1 or restarts <= 1:
        for restart in range(restarts):
            candidate = run_single_restart_dataset_1(dataset, restart, iterations, seed)
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        futures = [
            pool.submit(run_single_restart_dataset_1, dataset, restart, iterations, seed)
            for restart in range(restarts)
        ]
...
def run_solver(dataset_id: int, iterations: int, restarts: int, workers: int, write_solution: bool) -> List[Action]:
    if dataset_id != 1:
        raise ValueError("solver_dataset_1 ne supporte que --dataset 1")
    if workers < 1:
    ...
    dataset_path = ROOT_DIR / "datasets" / "dataset_1.json"
    if not dataset_path.exists():
        raise FileNotFoundError(f"Dataset introuvable: {dataset_path}")
    dataset = load_dataset(dataset_path)

    started = time.time()
    best_actions = find_best_solution_dataset_1(
        dataset=dataset,
    ...
    if write_solution:
        out = ROOT_DIR / "solutions" / "solution_1.txt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Solver dedie pour dataset_1")
    parser.add_argument("--dataset", type=int, default=1, choices=[1], help="Dataset number (must be 1)")
    ...
    parser.add_argument("--write", action="store_true", help="Write solution to solutions/solution_1.txt")
